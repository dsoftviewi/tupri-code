...
<html lang="en">
	<head>
		...
		<title>DVI Holidays | Order Pro</title>
 ...
 <body class="tooltips">
          ...
          <div class="page-content">
			...
			<!-- BEGIN PAGE CONTENT -->
            <?php include("AGENT/tourplan.php");?>
	...
	$.get('<?php echo 'AGENT/ref_cities.php' ?>', function(data) {
...
{
	$('.loader_ax').fadeIn();
	$.get('<?php echo 'AGENT/row_cities.php'; ?>?chek='+$('#check_boxs').val(), { incr1 : mrow, incr2 : srow, city_id: arr_city_id, arr_vehids: vehids }, function(data,sts,xh) {
		$('.loader_ax').fadeOut(500);
//		alert(data);
...
{
	$('.loader_ax').fadeIn();
	$.get('<?php echo 'AGENT/row_allcities.php' ?>', { incr1 : incdy, incr2 : cty }, function(data) {
		$('.loader_ax').fadeOut(500);
		//loading_hide_cityrow();
...
function get_cities3(mrow,srow,arr_city_id,vehids) 
{
	$('.tooltips').tooltip();
	$('.loader_ax').fadeIn();
	$.get('<?php echo 'AGENT/ref_cities1.php' ?>', { incr1 : mrow, incr2 : srow, city_id: arr_city_id, arr_vehids: vehids }, function(data,sts,xh) {
		$('.loader_ax').fadeOut(500);
//		alert(data);
		loading_hide_cityrow();
	$('#load_cityrow'+mrow+srow).html(data);
...
function getvehicles(city_id)
{
	...
	var inc=$('#vehdiv').val();
	$.get('<?php echo 'AGENT/ref_vehicl.php' ?>', { city_id: cit_id, incr : inc }, function(data) {
	...
	if (check_empty == '')
	{
		...
		/*for(var w1=0;w1<=dyplus;w1++)
		{
			...
			for(var x1=0;x1<=gethid;x1++)
			{
				if (gethid > 0)
				{
					var tocity_val = $('#row_city'+w1+x1+' option:selected').text().split('-');
					alert('>0');
					alert("value"+tocity_val);
					if (tocity_val != '')
					{
						var x2 = x1+1;
						alert('get val'+w1+x1);
						alert('put val'+w1+x2);
						$("#start_city"+w1+x2).val(tocity_val[0].trim());
						var x3 = x2;
					}
				}
				else if (gethid == 0)
				{
					var w2 = w1-1;
					var tocity_val = $('#row_city'+w2+x3+' option:selected').text().split('-');
					alert('=0');
					alert('get val'+w2+x3);
					alert('put val'+w1+x1);
					alert(tocity_val);
					$("#start_city"+w1+x1).val(tocity_val[0].trim());
				}
				
				
				if (typeof(tocity_val) != "undefined" && tocity_val != null && tocity_val != '')
				{
