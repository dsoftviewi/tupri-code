...
<!--		<link href="core/assets/plugins/validator/bootstrapValidator.min.css" rel="stylesheet">-->
 ...
 <body class="tooltips">
          ...
          <div class="page-content">
			...
			<!-- BEGIN PAGE CONTENT -->
            <?php include("AGENT/break_stay.php");?>
	...
	$.get('AGENT/ref_cities.php', function(data) {
...
<!--		<script src="core/assets/plugins/summernote/summernote.min.js"></script>
        ...
        <script src="core/assets/js/toastr-demo.js"></script>
	...
	$(document).ready(function() {
	 ...
	 <?php if(substr($_GET['id'],0,2)=='T#') {?>
			 $.get('AGENT/itin_submit_trav_report.php?planid=<?php echo urlencode($_GET['id']); ?>',function(result)
			 {
				 //alert(result);
	 ...
	 <?php }else if(substr($_GET['id'],0,2)=='TH'){?>
			...
			$.get('AGENT/itin_submit_trhot_report.php?planid=<?php echo urlencode($_GET['id']); ?>',function(result)
			 {
				 //alert(result);
				 $('#plan_det_info_modbody').html(result);
				 $('#plan_det_info_mod').modal('show');
...
{
	$('.loader_ax').fadeIn();
	$.get('AGENT/row_cities.php?chek='+$('#check_boxs').val(), { incr1 : mrow, incr2 : srow, city_id: arr_city_id, arr_vehids: vehids }, function(data,sts,xh) {
...
function get_cities3(mrow,srow,arr_city_id,vehids) 
{
	$('.tooltips').tooltip();
	$('.loader_ax').fadeIn();
	$.get('AGENT/ref_cities1.php', { incr1 : mrow, incr2 : srow, city_id: arr_city_id, arr_vehids: vehids }, function(data,sts,xh) {
		$('.loader_ax').fadeOut(500);
//		alert(data);
		$('.tooltips').tooltip();
		loading_hide_cityrow();
...
function getvehicles1()
{
	...
	var inc=$('#vehdiv').val();
	$('.loader_ax').fadeIn();
	$.get('AGENT/ref_vehicl.php', { city_id: cit_id, incr : inc }, function(data) {
	  $('.loader_ax').fadeOut(500);
	//alert(data);
