...
<?php
//mysql_select_db($database_divdb, $divdb);
$seo=$conn->prepare("SELECT * FROM seo_settings_new where type='HOT'");
$seo->execute();
...
{
	$htspots=$conn->prepare("SELECT * FROM hotspots_pro where  status='0' and spot_state=?  GROUP BY spot_city ASC");
}else{
	$htspots=$conn->prepare("SELECT * FROM hotspots_pro where  status='0' and spot_state=?  GROUP BY spot_city ASC LIMIT 10");
	
}

$htspots->execute(array($id));
...
$cities=$conn->prepare("SELECT * FROM dvi_cities where region = ? and id= ?");
$cities->execute(array($id,$row_htspots['spot_city']));
$row_cities = $cities->fetch(PDO::FETCH_ASSOC);
$total_cities = $cities->rowCount();			


				
$states1=$conn->prepare("SELECT * FROM dvi_states where code = ?");
$states1->execute(array($id));
$row_states1 = $states1->fetch(PDO::FETCH_ASSOC);
$total_states1 = $states1->rowCount();			
...
$numhtspots=$conn->prepare("SELECT * FROM hotspots_pro where  status='0' and spot_city= ?");
$numhtspots->execute(array($row_htspots['spot_city']));
$row_numhtspots = $numhtspots->fetch(PDO::FETCH_ASSOC);
$total_numhtspots = $numhtspots->rowCount();
