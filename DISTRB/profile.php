...
move_uploaded_file($_FILES["pro_pic"]["tmp_name"], $target_file);
 ...
 $update_pro=$conn->prepare("update distributor_pro set distr_fname=?,distr_lname=?,distr_addr=?,comp_name=?,busi_volume=?,state=?,city=?,land_line=?,mobile_no=?,email_id=?,fax_no=?,iata_no=?,bank_addr=?,bank_acc_no=?,bank_ifsc_code=?,distr_img=?,work_state=? where distr_id=?");
...
echo "<script>parent.location.reload();parent.fancybox.closse();</script>";
}
...
$dvistate->execute(array($_GET['uid']));
