...
$orders = $conn->prepare("SELECT * FROM travel_master where distr_id =? and sub_paln_id!='' ORDER BY sno DESC");
}else{
$orders = $conn->prepare("SELECT * FROM travel_master where distr_id =? and sub_paln_id!='' ORDER BY sno DESC LIMIT 100");	
}
...
if(isset($_GET['older']) && $_GET['older']=='yes'){
$thorders = $conn->prepare("SELECT * FROM travel_master where distr_id =? and sub_paln_id!='' and SUBSTR(plan_id,1,2) = 'TH' ORDER BY sno DESC");
}else{
$thorders = $conn->prepare("SELECT * FROM travel_master where distr_id =? and sub_paln_id!='' and SUBSTR(plan_id,1,2) = 'TH' ORDER BY sno DESC LIMIT 50");
}
...
if(isset($_GET['older']) && $_GET['older']=='yes'){
$torders = $conn->prepare("SELECT * FROM travel_master where distr_id =?  and sub_paln_id!='' and SUBSTR(plan_id,1,2) = 'T#' ORDER BY sno DESC");
}else{
	$torders = $conn->prepare("SELECT * FROM travel_master where distr_id =?  and sub_paln_id!='' and SUBSTR(plan_id,1,2) = 'T#' ORDER BY sno DESC LIMIT 50");
}
...
if(isset($_GET['older']) && $_GET['older']=='yes'){
$horders = $conn->prepare("SELECT * FROM travel_master where distr_id =?  and sub_paln_id!='' and SUBSTR(plan_id,1,2) = 'H#' ORDER BY sno DESC");
}else{
	$horders = $conn->prepare("SELECT * FROM travel_master where distr_id =?  and sub_paln_id!='' and SUBSTR(plan_id,1,2) = 'H#' ORDER BY sno DESC LIMIT 50");
}
...
.scroll-nav-dropdowns
{
	...
	width:240px;
              ...
              <li><a href="#tr_hot" data-toggle="tab"><i class="fa fa-building-o"></i> Travel + Hotel</a></li>
							...
							<li class="pull-right"><a href="distrb_orderstatus.php?mm=59a64762ef215e93af370c7d8cb4a01a&sm=0d020d1a6aa04515f0389af057a103b4&older=yes"> Show More </a></li>
							</ul>
						  </div>
     ...
     <div class="modal fade" id="resume_transmodal" tabindex="-1" role="dialog" aria-hidden="true" data-backdrop="static"  data-keyboard="false"  >
                                    ...
                                    <div class="modal-body" style="padding-top: 5px; padding-left: 5px;padding-right: 20px; padding-bottom: 5px; height:450px; overflow-y:scroll" id="resume_transmodal_body" ><!-- loading transport itinerary form itin_submit_trav_report.php -->
                                    </div>
                       ...
                       <button type="button" class="btn btn-danger"  onClick="manage_status1('cancel')">Cancel It</button>
                       <button type="button" class="btn btn-success" onClick="manage_status1('confirm')">Confirm It</button>
                        </div>
              ...
              <div class="modal fade" id="resume_modal" tabindex="-1" role="dialog" aria-hidden="true" data-backdrop="static"  data-keyboard="false"  >
                          ...
                          <div class="panel with-nav-tabs panel-info panel-square" id="rep_tabss">
						  <div class="panel-heading">
						  <ul class="nav nav-tabs">
								<li ><a href="#itin_rep" data-toggle="tab" id="resume_header" onclick="show_itinerary()">Itinerary Report</a></li>
								<li class="active" id="holistt"><a href="#hotel_listt" data-toggle="tab" onclick="show_func_dvisug()">Hotel Report</a></li>
										...
										Please Wait ...
                                          ...
                                          <div class="row" style="margin-top:10px;">
                                             ...
                                             <input type="text" class="form-control col-sm-4" placeholder="Guest name" name="guestname1" id="guestname1">
                                              ...
                                              <div class="col-sm-11" align="center">
                       <button type="button" class="btn btn-danger"  onClick="manage_status('cancel')">Cancel It</button>
                       <button type="button" class="btn btn-success" id="sugg_confirm" onClick="manage_status('confirm')">Confirm It</button>
                       <button type="button" class="btn btn-success" id="pow_confirm" onClick="manage_status_pow()" style="display:none;">Confirm It</button>
                        </div>
                        <div class="col-sm-1" align="right">
                        <button type="button" class="btn btn-default " data-dismiss="modal" >Close</button></div>
											  </div>
									...
									<div class="tab-content">
										<div class="tab-pane fade in active" id="view_all">
											...
											?>
												...
												<table class="table table-striped table-hover table-default table-th-block datatable-example" id="">
													<thead class="the-box dark full">
														<tr>
															<th>#</th>
															<th><i class="fa fa-ticket"></i>&nbsp; Travel id</th>
                                                            <th><i class="fa fa-calendar-o"></i>&nbsp; Booking date</th>
															<th><i class="fa fa-user"></i>&nbsp; Creator info</th>
													...
													foreach($row_orders_main as $row_orders)
													{
														...
														$distrb->execute(array($_SESSION['uid']));
														$row_distrb = $distrb->fetch(PDO::FETCH_ASSOC);
														
														if($row_distrb['is_super'] == 1)
														{
															$cod_nm = 'my_report.php';
															$grp='ADMIN';
														}
														elseif($row_distrb['is_super'] == 0)
														{
															$cod_nm = 'my_agn_report.php';
															$grp=$_SESSION['grp'];
														}
														...
														<tr class="odd gradeX">
                                                            ...
                                                            <?php } 
																...
																<a  href="javascript:void(0);" data-toggle='tooltip' class=" badge  badge-default  tooltips pull-right"  data-original-title='No vouchers for this plan'><i class="fa fa-vine"></i></a>
															<?php }?>
                                                            ...
                                                            <a class="show_pdf badge pull-right badge-info " data-toggle='tooltip' href="<?php echo $grp.'/'.$cod_nm;?>?mm=<?php echo $_GET['mm']; ?>&sm=<?php echo $_GET['sm'];?>&planid=<?php echo urlencode($row_orders['plan_id']); ?>" data-original-title='<?php echo $row_orders['plan_id'].'- Plan Info'; ?>'><i class="fa fa-file-text"></i></a>
                                                            
                                                            </td>
															<td>
															<?php if (($row_orders['agent_id'] != '-') && ($row_orders['agent_id'] != '')) 
															{ ?>
															<span data-toggle='tooltip' data-original-title='<?php echo "From ".$row_agncity['name'].' - '.$row_agnstat['name']; ?>'><?php echo $row_agnt['agent_fname'].' '.$row_agnt['agent_lname'];  ?></span>
															<?php } else {?> 
                                                            <span data-toggle='tooltip' data-original-title='Created By You'>You</span>
																 <?php } ?></td>
											 ...
											 if($id_mas =='TH')
											 {
                                                            ...
                                                            <?php }else{?>
															<a class="pull-right badge badge-warning" data-toggle='tooltip' data-original-title='Rejected/Cancelled' target="_blank" href="AGENT/trvl_mailing.php?planid=<?php echo urlencode($row_orders['plan_id']); ?>"><i class="fa fa-envelope"></i></a>	
															<?php }
											 }else if($id_mas =='T#'){ 
                                                           if($row_orders['status']==0 || $row_orders['status']==2 || $row_orders['status']==5) {?>
															...
															if($id_mas =='TH')
															{?>
																<a class="show_pdf btn btn-info btn-sm" style="width:116px" title="Plan details" href="<?php echo 'ADMIN';?>/itiner_trav_hotel.php?mm=<?php echo $_GET['mm'];?>&sm=<?php echo $_GET['sm'];?>&planid=<?php echo urlencode($row_orders['plan_id']); ?>"><i class="fa fa-building-o"></i> Plan detail</a>
															<?php }else if($id_mas =='H#')
															{?>
																<a class="show_pdf btn btn-info btn-sm" style="width:116px"  title="Plan details" href="<?php echo $_SESSION['grp'];?>/itiner_hotel.php?mm=<?php echo $_GET['mm'];?>&sm=<?php echo $_GET['sm'];?>&planid=<?php echo urlencode($row_orders['plan_id']); ?>"><i class="fa fa-home"></i></i> Plan detail</a>
															<?php }else if($id_mas =='T#')
															{?>
																<a class="show_pdf btn btn-info btn-sm" style="width:116px"   title="Plan details" href="<?php echo 'ADMIN';?>/itiner_trav.php?mm=<?php echo $_GET['mm'];?>&sm=<?php echo $_GET['sm'];?>&planid=<?php echo urlencode($row_orders['plan_id']); ?>"><i class="fa fa-automobile"></i> Plan detail</a>
															<?php }
															?>
															</td>
															<td class="center">
															<?php
											if ((($row_orders['agent_id'] == '-') || ($row_orders['agent_id'] == '')) && $row_orders['status'] ==4)
															{
															?>
														...
														<span  class="form-control" style="background-color: #F1DFD8;color: #CE470D;font-weight: 600;" data-toggle='tooltip' data-original-title='Rejected Itinerary'><i class="fa fa-thumbs-down"></i> &nbsp;Rejected &nbsp;&nbsp;&nbsp; </span>
															...
															<?php
															}elseif($row_orders['status'] == 5 && ($row_orders['agent_id'] == '-') || ($row_orders['agent_id'] == ''))
															{
																?>
															<span class="form-control btn" style="background-color: #EAE0C4;color: #674016;font-weight: 600;"  data-toggle='tooltip' data-original-title='Waiting to resume' onclick="resume_me('<?php echo $row_orders['plan_id']; ?>')"><i class="fa fa-play"></i> &nbsp;&nbsp;Resume&nbsp;&nbsp;&nbsp; </span> 
																<?php
															}else if($row_orders['status'] == 5 && ($row_orders['agent_id'] != '-') || ($row_orders['agent_id'] != '')){?>
																<span class="form-control " style="background-color: #DBE2EA;color: #29345A;font-weight: 600;"  data-toggle='tooltip' data-original-title='Itinerary Saved By Agent'><i class="fa fa-pause"></i> &nbsp;&nbsp;Saved &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; </span>
														<?php 	}
															?>
                                                            </td>
														</tr>
											...
											?>
												...
												<table class="table table-striped table-hover table-default table-th-block datatable-example" id="">
													...
													foreach($row_thorders_main as $row_thorders)
													{
														...
														$distrb1 = $conn->prepare("SELECT * FROM distributor_pro WHERE distr_id =?");
														$distrb1->execute(array($_SESSION['uid']));
														$row_distrb1 = $distrb1->fetch(PDO::FETCH_ASSOC);
														
														if($row_distrb1['is_super'] == 1)
														{
															$cod_nm1 = 'my_report.php';
															$grp='ADMIN';
														}
														elseif($row_distrb1['is_super'] == 0)
														{
															$cod_nm1 = 'my_agn_report.php';
															$grp=$_SESSION['grp'];
														}
														...
														<tr class="odd gradeX">
                                                            ...
                                                            <a class="show_pdf badge badge-info pull-right" data-toggle='tooltip' data-original-title='<?php echo $row_thorders['plan_id'].' - Plan Info'; ?>' href="<?php echo $grp.'/'.$cod_nm1;?>?mm=<?php echo $_GET['mm']; ?>&sm=<?php echo $_GET['sm'];?>&planid=<?php echo urlencode($row_thorders['plan_id']); ?>"><i class="fa fa-file-text"></i></a>
                                                            
                                                            </td>
                                                            
                                                            <td>
															<?php if (($row_thorders['agent_id'] != '-') && ($row_thorders['agent_id'] != '')) 
															{ ?>
															<span data-toggle='tooltip' data-original-title='<?php echo "From ".$row_agncity1['name'].' - '.$row_agnstat1['name']; ?>'><?php echo $row_agnt1['agent_fname'].' '.$row_agnt1['agent_lname'];  ?></span>
															<?php } else {?> 
                                                            <span data-toggle='tooltip' data-original-title='Created By You'>You</span>
																 <?php } ?></td>
                                                            ...
                                                            <!-- Mail start -->
														<?php    if($row_thorders['status']==0 || $row_thorders['status']==2 || $row_thorders['status']==4) {?>
											 ...
											 <!-- Mail end -->
															...
															if($id_mas =='TH')
															{?>
																<a class="show_pdf btn btn-info btn-sm"  title="Plan details" href="<?php echo 'ADMIN';?>/itiner_trav_hotel.php?mm=<?php echo $_GET['mm'];?>&sm=<?php echo $_GET['sm'];?>&planid=<?php echo urlencode($row_thorders['plan_id']); ?>"><i class="fa fa-building-o"></i> Plan detail</a>
															<?php } ?>
															</td>
                                                            <td class="center">
															<?php
											if ((($row_thorders['agent_id'] == '-') || ($row_thorders['agent_id'] == '')) && $row_thorders['status'] ==4)
															{
															?>
														...
														<span  class="form-control" style="background-color: #F1DFD8;color: #CE470D;font-weight: 600;" data-toggle='tooltip' data-original-title='Rejected Itinerary'><i class="fa fa-thumbs-down"></i> &nbsp;Rejected &nbsp;&nbsp;&nbsp; </span>
															...
															}elseif($row_thorders['status'] == 3)
															{
																?>
																	<span class="form-control" style="background-color: #F7F5CF;color: #847B08;font-weight: 600;"  data-toggle='tooltip' data-original-title='Cancelled By Customer'><i class="fa fa-times"></i> &nbsp;Cancelled &nbsp;&nbsp; </span> 
															<?php
															}elseif($row_thorders['status'] == 5 && ($row_thorders['agent_id'] == '-') || ($row_thorders['agent_id'] == ''))
															{
																?>
															<span class="form-control btn" style="background-color: #EAE0C4;color: #674016;font-weight: 600;"  data-toggle='tooltip' data-original-title='Waiting to resume' onclick="resume_me('<?php echo $row_thorders['plan_id']; ?>')"><i class="fa fa-play"></i> &nbsp;&nbsp;Resume&nbsp;&nbsp;&nbsp; </span> 
																<?php
															}else if($row_thorders['status'] == 5 && ($row_thorders['agent_id'] != '-') || ($row_thorders['agent_id'] != '')){?>
																<span class="form-control " style="background-color: #DBE2EA;color: #29345A;font-weight: 600;"  data-toggle='tooltip' data-original-title='Itinerary Saved By Agent'><i class="fa fa-pause"></i> &nbsp;&nbsp;Saved &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; </span>
														<?php 	}
															?>
                                                            </td>
												...
												<table class="table table-striped table-hover table-default table-th-block datatable-example" id="">
													<thead class="the-box dark full">
														<tr>
															<th>#</th>
															<th><i class="fa fa-ticket"></i>&nbsp; Travel id</th>
                                                            <th><i class="fa fa-calender-o"></i>&nbsp; Booking date</th>
															<th><i class="fa fa-user"></i>&nbsp; Agent info</th>
															<th><i class="fa fa-suitcase"></i>&nbsp; Traveller info</th>
															<th><i class="fa fa-paper-plane"></i>&nbsp; Plan info</th>
															<th><i class="fa fa-support"></i>&nbsp; Status</th>
													...
													foreach($row_torders_main as $row_torders)
													{
														...
														$distrb2 = $conn->prepare("SELECT * FROM distributor_pro WHERE distr_id =?");
														$distrb2->execute(array($_SESSION['uid']));
														$row_distrb2 = $distrb2->fetch(PDO::FETCH_ASSOC);
														
														if($row_distrb2['is_super'] == 1)
														{
															$cod_nm2 = 'my_report.php';
															$grp='ADMIN';
														}
														elseif($row_distrb2['is_super'] == 0)
														{
															$cod_nm2 = 'my_agn_report.php';
															$grp=$_SESSION['grp'];
														}
														...
														<tr class="odd gradeX">
                                                            ...
                                                            <a class="show_pdf badge badge-info pull-right" data-toggle='tooltip' data-original-title='<?php echo $row_torders['plan_id'].' - Plan Info' ?>' href="<?php echo $grp.'/'.$cod_nm2;?>?mm=<?php echo $_GET['mm']; ?>&sm=<?php echo $_GET['sm'];?>&planid=<?php echo urlencode($row_torders['plan_id']); ?>"><i class="fa fa-file-text"></i></a>
                                                            </td>
                                                            
                                                            <td>
															<?php if (($row_torders['agent_id'] != '-') && ($row_torders['agent_id'] != '')) 
															{ ?>
															<span data-toggle='tooltip' data-original-title='<?php echo "From ".$row_agncity2['name'].' - '.$row_agnstat2['name']; ?>'><?php echo $row_agnt2['agent_fname'].' '.$row_agnt2['agent_lname'];  ?></span>
															<?php } else {?> 
                                                            <span data-toggle='tooltip' data-original-title='Created By You'>You</span>
																 <?php } ?></td>
                                                            ...
                                                            <!-- mail start -->   
                                                           <?php 
														   if($row_torders['status']=='0' || $row_torders['status']=='2' || $row_torders['status']=='5' || $row_torders['status']=='4')
														   {?>
															  <a class="pull-right badge badge-info" data-toggle='tooltip' data-original-title='Send Mail' target="_blank" href="AGENT/trvl_mailing.php?planid=<?php echo urlencode($row_torders['plan_id']); ?>"><i class="fa fa-envelope"></i></a> 
														  <?php  }else{ ?>
															   <a class="pull-right badge badge-default" data-toggle='tooltip' data-original-title='Cancelled' href="javascript:void(0);"><i class="fa fa-envelope"></i></a>
														   <?php }
														   ?>
                                    <!-- Mail End -->
															...
															if($id_mas =='T#')
															{?>
                                                            <a class="show_pdf btn btn-info btn-sm"  title="Plan details" href="<?php echo 'ADMIN';?>/itiner_trav.php?mm=<?php echo $_GET['mm'];?>&sm=<?php echo $_GET['sm'];?>&planid=<?php echo urlencode($row_torders['plan_id']); ?>"><i class="fa fa-automobile"></i> Plan detail</a>
															<?php } ?>
															</td>
															
                                                            <td class="center">
															<?php
											if ((($row_torders['agent_id'] == '-') || ($row_torders['agent_id'] == '')) && $row_torders['status'] ==4)
															{
															?>
														...
														<span  class="form-control" style="background-color: #F1DFD8;color: #CE470D;font-weight: 600;" data-toggle='tooltip' data-original-title='Rejected Itinerary'><i class="fa fa-thumbs-down"></i> &nbsp;Rejected &nbsp;&nbsp;&nbsp; </span>
															...
															}elseif($row_torders['status'] == 3)
															{
																?>
																	<span class="form-control" style="background-color: #F7F5CF;color: #847B08;font-weight: 600;"  data-toggle='tooltip' data-original-title='Cancelled By Customer'><i class="fa fa-times"></i> &nbsp;Cancelled &nbsp;&nbsp; </span> 
															<?php
															}elseif($row_torders['status'] == 5 && ($row_torders['agent_id'] == '-') || ($row_torders['agent_id'] == ''))
															{
																?>
															<span class="form-control btn" style="background-color: #EAE0C4;color: #674016;font-weight: 600;"  data-toggle='tooltip' data-original-title='Waiting to resume' onclick="resume_me('<?php echo $row_torders['plan_id']; ?>')"><i class="fa fa-play"></i> &nbsp;&nbsp;Resume&nbsp;&nbsp;&nbsp; </span> 
																<?php
															}else if($row_torders['status'] == 5 && ($row_torders['agent_id'] != '-') || ($row_torders['agent_id'] != '')){?>
																<span class="form-control " style="background-color: #DBE2EA;color: #29345A;font-weight: 600;"  data-toggle='tooltip' data-original-title='Itinerary Saved By Agent'><i class="fa fa-pause"></i> &nbsp;&nbsp;Saved &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; </span>
														<?php 	}
															?>
                                                            </td>
...
function houseboat_editable(hid,ctg,no)
{
	...
	$('.loader_ax').fadeIn();
	$.get('AGENT/resume_itin_dvi_sugg.php?plan_id1='+planid[0]+'&plan_id2='+planid[1]+'&edit_ctg='+ctg+'&edit_hid='+hid+'&edit_no='+no,function(res)
		{
			$('.loader_ax').fadeOut();
...
function resume_me(pid)
{
	...
	if(planid[0]=='TH')
	{
		...
		$.get('AGENT/resume_itin.php?plan_id1='+planid[0]+'&plan_id2='+planid[1]+'&res=show',function(res)
		{
			...
			$('#pow_hotel').attr('onclick','plan_on_own(),hide_func_dvisug()');
					...
					$.get('AGENT/resume_rep_details.php?plan_id1='+planid[0]+'&plan_id2='+planid[1],function(ress){
						$('#itin_rep').empty().html(ress);
					});
		});
	}else if(planid[0]=='T')
	{
		$.get('AGENT/itin_submit_trav_report.php?plan_id1='+planid[0]+'&plan_id2='+planid[1]+'&res=show',function(res)
		{
...
{
	var room_inf=$('#room_info_pow').val().trim();
	var num_sty_rooms=$('#stay_rooms_pow').val().trim();
	var cities_id=$('#city_ids_pow').val().trim();
	var cities_name=$('#city_idname_pow').val().trim();
	var strt_date=$('#stdate_pow').val().trim();
	...
	$('#sdate_nw_td'+days).empty().html("<input class='form-control' type='text' readonly id='sdat"+days+"' name='sdat"+days+"' value='"+daysss+"'>");
		$('#city_nw_td'+days).empty().html("<input type='text' class='form-control' value='"+city_kitname+"' id='kitcity"+eat+"' readonly='readonly' >");
			}
...
/*function manage_status_pow_confirm()
{
	...
	if ($('#guestname1').val().trim() == '' || $('#mobil1').val().trim() == '' || $('#arrdet1').val().trim() == '' || $('#depdet1').val().trim() == '') 
		...
		}else if($('#depdet1').val().length<4)
		{
			...
			var strt_datepowd=$('#stdate_pow').val().trim();
			...
			var datastring = $("#resume_me_form").serialize();
									$.ajax({
										type: "POST",
										url: "AGENT/resume_planonown.php?plan_id1="+spidd[0]+"&plan_id2="+spidd[1]+"&day_of_stay="+day_cntpowd,
										data: datastring,
										success: function(res) {
											alert(res);
											$('#resume_modal').modal('hide');
...
/*function download_vouchers(pid,trn)
{
	var pidd=pid.split('#');
	$('.loader_ax').fadeIn();
	$.get('ADMIN/ajax_voucher.php?type=1&pid1='+pidd[0]+'&pid2='+pidd[1]+'&trname='+trn,function(res){
				$('.loader_ax').fadeOut(500);
...
function download_vouchers(pid,trn)
{
  var pidd=pid.split('#');
  $('.loader_ax').fadeIn();
  $.get('AGENT/ajax_voucher.php?type=1&pid1='+pidd[0]+'&pid2='+pidd[1]+'&trname='+trn,function(res){
        $('.loader_ax').fadeOut(500);
        $('#download_vouch').empty().html(res);
        $('#download_vouch').modal('show');
    });
