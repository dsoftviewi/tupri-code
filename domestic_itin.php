...
{
	$package = $conn->prepare("SELECT * FROM dvi_packages where status='0' and pack_grp='DI' ORDER BY pack_prior ASC");
}else{
	$package = $conn->prepare("SELECT * FROM dvi_packages where status='0' and pack_grp='DI' ORDER BY pack_prior ASC LIMIT 10");
}
$package->execute();
...
<a <?php echo 'href="core/ajax/packages_slide.php?'. http_build_query(array('cluster' => $img_view)). '"'; ?> class="hover-effect popup-gallery"><img src="packages/images/<?php echo $img_view; ?>" alt="tour packages for south india" title="travel agencies in trichy" style="height:170px; width:290px" /></a>
...
<a class="button btn-small full-width text-center" style="background-color: rgb(148, 183, 54);" href="view_package.php?pack_id=<?php echo $row_package['sno']; ?>">View</a>
...
<img src="images/logo.png" alt="tour packages for south india" title="travel agencies in trichy" />
</div></div>
