...
mysql_select_db($database_exam, $exam);
			$query_cerinfo= "SELECT * FROM setting_certificate where  status!='1' ";
...
move_uploaded_file($_FILES["signs"]["tmp_name"],$newname2);
 ...
 $insertSQL = sprintf("INSERT INTO setting_certificate (`title`,`certify_logo`,`sub_title`,`auth_desg`,`signature`,`line_one`,`line_two`,`line_tre` ) VALUES (%s, %s,%s,%s,%s,%s,%s,%s)",
					   GetSQLValueString($_POST['cer_title'], "text"),
					   GetSQLValueString($newname1, "text"),
					   GetSQLValueString($_POST['sb_title'], "text"),
					   GetSQLValueString($_POST['auth_desgs'], "text"),
					   GetSQLValueString($newname2, "text"),
					   GetSQLValueString($_POST['lineone'], "text"),
					   GetSQLValueString($_POST['linetwo'], "text"),
					   GetSQLValueString($_POST['linetre'], "text"));

  mysql_select_db($database_exam, $exam);
  $Result = mysql_query($insertSQL, $exam) or die(mysql_error());


$msg = "The new Certificate setup has been enrolled by the Admin";
  activity($msg,$database_exam, $exam, 'fa fa-plus', 'completed', 'Enrolled', $_COOKIE['uid']);
 
 $s='setup';
 $ts='Certificate';
 echo "<script>parent.document.location.href='../admin_examsetting.php?mm=".$_GET['mm']."&sm=".$_GET['sm']."&alt=".md5(1)."&name=". $s."&t=".$ts."';</script>"; 
