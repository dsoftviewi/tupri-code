...
if ($_GET['route_type'] == 'm')
{
	...
	*/
	...
	//$tot_dist = $tot_dist + 80;
	
	
	//via calcation - adding sight-seeing distance to total distance
	//echo $viass;
	// print_r($viass);
	$via_ssee=0;
	if($viass != '-')
	{
		//print_r($viass);
			...
			for($v1=0;$v1<count($viass);$v1++)
			{
				$vss1 = $conn->prepare("SELECT * FROM dvi_cities where id=?");
				$vss1->execute(array($viass[$v1]));
				$row_vss1 = $vss1->fetch(PDO::FETCH_ASSOC);
				$via_ssee=$via_ssee+(int)$row_vss1['ss_dist'];
			}
	}
	
	$via_ttol=0;
	$viass_dd_arr=explode('-',$_GET['viass_dd']);
	$trav_dist_arr=explode(',',$trav_dist);
	//print_r($trav_dist_arr);
	$trav_dist_ess_arr=explode(',',$trav_dist_ess);
//print_r($trav_dist_ess_arr);
	if(isset($viass_dd_arr[0]))
	{
		//print_r($viass_dd_arr);
		for($vd=0;$vd<count($viass_dd_arr);$vd++)
		{
			if($viass_dd_arr[$vd]!='NN')
			{
			$ss_dd=(int)$trav_dist_arr[$vd]-(int)$trav_dist_ess_arr[$vd];
			$trav_dist_ess_arr[$vd]=$viass_dd_arr[$vd];
			$trav_dist_arr[$vd]=$viass_dd_arr[$vd]+$ss_dd;
				$via_ttol=$via_ttol+(int)$viass_dd_arr[$vd];
			}else{
				$ss_dd=(int)$trav_dist_arr[$vd]-(int)$trav_dist_ess_arr[$vd];
				$via_ttol=$via_ttol+(int)$trav_dist_ess_arr[$vd]+$ss_dd;
			}
		}
	}
	//print_r($send_edist_ess_arr);
	
	//echo "  ______  ";
	//print_r($send_edist_arr);
	//echo "***** ".$via_ttol;
	 $trav_dist=implode(',',$trav_dist_arr);
	 $trav_dist_ess=implode(',',$trav_dist_ess_arr);
	
	// $send_dist = $send_dist + 80;
	//$send_dist = $send_dist + $via_ssee+ 80; 
	$trav_dist_tt =$via_ttol+$via_ssee;
	$trav_dist_ess_tt=$via_ttol;
///echo $trav_dist_tt;
	
	echo json_encode(array("trav_dist" => "$trav_dist", "trav_dist_ess" => "$trav_dist_ess", "trav_totdist" => "$trav_dist_tt", "trav_totdist_ess" => "$trav_dist_ess_tt", "extdist" => "$extdis_str", "frmt_time" => "$frmt_time", "cit_ord" => "$cit_ord", "cit_opt_idd" => "$cit_optid2","cit_optnnmm" => "$city_opt_nnn1")); 
}
elseif ($_GET['route_type'] == 'o')
{
	//echo "OOO";
	//echo "  BEFORE  ";
	//print_r($_GET['allcids']);
	$my_sight=0;
	$arrcid = $arrcid1 = $_GET['allcids'];
//	$arrcid = $arrcid1 = array('35','22','27','14','25','38','11','20','4','19','16','61','4');
	//print_r($arrcid);
	$arrveh_cid = $_GET['vehcitids'];
	$last_chk = $_GET['last_chk'];
	$v=1; $sh_cid = '';
	$reord = array(); $opt_fail = '';
	//print_r($arrcid);
	...
	//Get count of number of start and end cities only.
	//print_r($arrcid);
	if($arrcid[0] != $arrcid[count($arrcid)-1])
	{
	...
	//permutation of all waypoint cities if cities <= 7. If itineraries has more than 7 cities, then split into groups of 5 cities and permutate. After getting permutation of one group, find the shortest path of the set and pass the end city of this set to start of next group permutation... Loop till all cities complete.
	...
	if (!empty($fin_arrcid))
	{
		...
		else
		{
				...
				foreach($send_fin as $pass_vals)
				{
					...
					for ($a=0;$a<count($exp_pass)-1;$a++)
					{
						...
						if($totalRows_citdist1 > 0)
						{
							...
							$myss1 = $conn->prepare("SELECT * FROM dvi_cities where id=?");
							$myss1->execute(array($exp_pass[$b]));
							$row_myss1 = $myss1->fetch(PDO::FETCH_ASSOC);
							$totalRows_myss1 = $myss1->rowCount();
							$my_sight=$my_sight+(int)$row_myss1['ss_dist'];
			...
			$new_final1 = array(); $tempr_var = 0;
			//print_r($new_final);
			foreach($new_final as $new_fin1_val)
			{
				foreach($fin_arrcid_cnt as $fin_key=>$fin_arr_val)
				{
					if($new_fin1_val == $fin_key)
					{
						for($zj=0;$zj<$fin_arr_val;$zj++)
						{
							$new_final1[$tempr_var] = $fin_key;
							$tempr_var++;
						}
					}
			...
			array_unshift($new_final1, $arrcid1[0]);
			array_push($new_final1, $arrcid1[count($arrcid1)-1]);
			//print_r($new_final1);
			...
			for ($a=0;$a<count($new_final1)-1;$a++)
			{
				...
				if($totalRows_citdist1 > 0)
				{
					if ($row_citdist1['dist'] > $row_chkrow['opt_dist'])
					{
						$opt_fail = 'y';
					}
				}
				else
				{
					...
					if($totalRows_citdist2 > 0)
					{
						if ($row_citdist2['dist'] > $row_chkrow['opt_dist'])
						{
							$opt_fail = 'y';
						}
					}
	...
	if ($comp_flag == '')
	{
		if (!empty($pass_arrcid))
		{
			...
			//print_r($finarr_dist);
			//echo  "____________";
			//print_r($finarr_dist_ess);
			$index = array_search(min($finarr_dist_ess), $finarr_dist_ess);
			$pass_arrcid1 = explode(',',$pass_arrcid[$index]);
			//echo $pass_arrcid[$index];
			//print_r($pass_arrcid1);
			
	
			for ($a=0;$a<count($pass_arrcid1)-1;$a++)
			{
				$b = $a + 1;
				
				
				$citdist1 = $conn->prepare("SELECT * FROM dvi_citydist where from_cityid=? and to_cityid =?");
				$citdist1->execute(array($pass_arrcid1[$a],$pass_arrcid1[$b]));
				$row_citdist1 = $citdist1->fetch(PDO::FETCH_ASSOC);
				$totalRows_citdist1 = $citdist1->rowCount();
				...
				else
				{
					...
					$citdist2 = $conn->prepare("SELECT * FROM dvi_citydist where from_cityid=? and to_cityid =?");
					$citdist2->execute(array($pass_arrcid1[$b],$pass_arrcid1[$a]));
					$row_citdist2 = $citdist2->fetch(PDO::FETCH_ASSOC);
					$totalRows_citdist2 = $citdist2->rowCount();
			...
			else
			{
				...
				for ($a=0;$a<count($pass_arrcid1)-1;$a++)
				{
					...
					if($totalRows_citdist1 > 0)
					{
						...
						$ss1 = $conn->prepare("SELECT * FROM dvi_cities where id=?");
						$ss1->execute(array($pass_arrcid1[$b]));
						$row_ss1 = $ss1->fetch(PDO::FETCH_ASSOC);
						$totalRows_ss1 = $ss1->rowCount();
					...
					else
					{
						...
						if($totalRows_citdist2 > 0)
						{
							...
							$ss2 = $conn->prepare("SELECT * FROM dvi_cities where id=?");
							$ss2->execute(array($pass_arrcid1[$b]));
							$row_ss2 = $ss2->fetch(PDO::FETCH_ASSOC);
							$totalRows_ss2 = $ss2->rowCount();
	...
	*/
	...
	$via_ttol=0;
	//echo 'vv'.$_GET['viass_dd'];
	$viass_dd_arr=explode('-',$_GET['viass_dd']);
	$send_edist_arr=explode(',',$send_edist);
	//print_r($send_edist_arr);
	$send_edist_ess_arr=explode(',',$send_edist_ess);
	if(isset($viass_dd_arr[0]))
	{
		for($vd=0;$vd<count($viass_dd_arr);$vd++)
		{
			if($viass_dd_arr[$vd]!='NN')
			{
			$ss_dd=(int)$send_edist_arr[$vd]-(int)$send_edist_ess_arr[$vd];
			$send_edist_ess_arr[$vd]=$viass_dd_arr[$vd];
			$send_edist_arr[$vd]=$viass_dd_arr[$vd]+$ss_dd;
				$via_ttol=$via_ttol+(int)$viass_dd_arr[$vd];
			}else{
				$ss_dd=(int)$send_edist_arr[$vd]-(int)$send_edist_ess_arr[$vd];
				$via_ttol=$via_ttol+(int)$send_edist_ess_arr[$vd]+$ss_dd;
			}
		}
	}
	//echo "My sight1 ".$send_dist;
	 $send_edist=implode(',',$send_edist_arr);
	 $send_edist_ess=implode(',',$send_edist_ess_arr);
	
	//$send_dist = $send_dist + 80;
	//$send_dist = $send_dist + $via_ssee+ 80; 
	$send_dist =$via_ttol+$via_ssee;
	$send_dist_ess=$via_ttol;

//echo "My sight ".$send_dist;
	echo json_encode(array("trav_dist" => "$send_edist", "trav_dist_ess" => "$send_edist_ess", "trav_totdist" => "$send_dist", "trav_totdist_ess" => "$send_dist_ess", "frmt_time" => "$frmt_time", "extdist" => "$extdis_str", "cit_ord" => "$cit_optord", "cit_opt_idd" => "$cit_optid2", "cit_optnams" => "$cit_optnams" , "cit_optnnmm" => "$city_opt_nnn", "opt_fail" => "$opt_fail")); 
