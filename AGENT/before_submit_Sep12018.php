...
//count($itiner_city2);
		...
		for($vcnt=0;$vcnt<count($veh_upl1)-1;$vcnt++)
		{
			...
			$insertSQL5->execute(array($tid,$veh_upl2[0],$veh_upl2[1],$veh_upl2[2],$veh_upl2[3],$veh_upl2[4],$veh_upl2[5],$veh_upl2[6],$veh_upl2[7],$veh_upl2[8],$veh_upl2[9]));
		}
		...
		foreach($exp_allveh as $each_veh_det)
		{
			...
			for($evcnt=0;$evcnt<count($exp_get_veh)-1;$evcnt++)
			{
				...
				$insertSQL6->execute(array($tid,$each_cityid,$each_veh_upl[0],$each_veh_upl[1],$each_veh_upl[2],$each_veh_upl[3],$each_veh_upl[4],$each_veh_upl[5],$each_veh_upl[6],$each_veh_upl[7],$each_veh_upl[8],$each_veh_upl[9]));
			}
		}
	...
	//for transpot+hotel
		...
		$insertSQL1 = $conn->prepare("INSERT INTO travel_master (plan_id, agent_id, distr_id, tr_name, tr_mobile, tr_arrdet, tr_depdet, pax_cnt, tr_arr_date, tr_arr_time, trv_depatr_time, pax_adults, pax_512child, pax_child, tr_days, tr_nights, arr_cityid, dest_cityid, tr_vehids, tr_vehname, tr_veh_cityid, veh_tot_rent, dt_cid, dt_detls, tot_tr_dist, tot_tr_dist_ess, dt_tot_dist, dt_trdist_ess, dt_ss_dist, tr_return_dist, net_tr_dist, perm_cityid, permit_amt, tr_net_amt, stay_rooms, stay_tot_amt, grand_tot, agent_perc, agnt_grand_tot, agnt_adm_perc, date_of_reg, status) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?, '5')");
