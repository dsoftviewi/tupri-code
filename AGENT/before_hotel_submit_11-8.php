		...
		/*	$shchild512=$_POST['youth_sel'.$h1];
			...
			}else if($check_hpro=='HOUSEBOAT' || $check_hpro=='House Boat')
			{
				...
				for($hb2=0;$hb2<=$loop;$hb2++)
				{
					...
					if($_POST[$frm.'_sel_hb_nw_extr'.$h1.'_'.$hb2]=='0')
					{
						$rate_for_child_bed=$rate_for_child_bed+$_POST[$frm.'_withbed_rate'.$h1];
					}else if($_POST[$frm.'_sel_hb_nw_extr'.$h1.'_'.$hb2]=='1')
					{
						$rate_for_child_bed=$rate_for_child_bed+$_POST[$frm.'_withoutbed_rate'.$h1];
					}else if($_POST[$frm.'_sel_hb_nw_extr'.$h1.'_'.$hb2]=='-')
					{
						$rate_for_child_bed=$rate_for_child_bed+0;
					}
				}
	...
	border:#666 solid 1px;
                            ...
                            <hr />
								<div class="row" style="padding:6px; text-align:center;border-top: 2px solid #E4AD69">
                                	<style="color:#C75A18"> Your Itinerary - will bypassing to below mentioned days [ Break-Stay ] 
                                </div>
							<?php }
							//////////
							?>
							<div class="row">
								...
								{ ?>
									<!--<div class="col-sm-12" style="padding:6px">
											<center><style="color:#6B4D1E; font-size: 22px;">Overall Package Cost : 
												<?php echo $whole_itin_amt."/- Only"; ?></center>
									</div>-->
...
color: whitesmoke; padding: 5px;"><?php echo "Itinerary Information "; ?></th></tr>
		...
		<tr>
			...
			<th style="padding:6px; font-family:Calibri; font-size: 14px;">
					<?php echo 	"Rs. ".number_format($row_or['agnt_grand_tot'],2); 
		...
		<tr>
			...
			<th style="padding:6px; font-family:Calibri; font-size: 14px;background-color: rgb(227, 241, 254);"><?php echo "Rs. ".number_format($whole_pakcost,2)." /-"; ?></th>
		</tr>
	</table>
...
//print_r($addon_arr);
	...
	<div style="margin-left:30px; border-bottom:2px dotted rgb(102, 74, 34);">
		<p align="left" style="margin-top:5px"><style="font-size: 12px;color: #005598;"> Total Cost Including Special Add-ons On : </p>
...
$totalRows_via_hspots =$via_hspots->rowCount();	
						...
						{//between 6am to 3pm ( over night in same city)
							echo "Greet and meet on arrival at ".$row_trv['tr_from_cityid']." and proceed to hotel, check-in hotel if time permits visit local and return to hotel and overnight at ".$row_hotel2['hotel_name'];//have to show sight-seeing in next day
							$show_in_next_day=2;
