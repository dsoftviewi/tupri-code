...
table td{
	padding:3px;
                  ...
                  <?php
					if($addi_addons_check==0){

				   if(isset($_GET['res']) && $_GET['res']=='show')
					{
				   ?>
                   <form name="resume_trav_addi_cost" id="resume_trav_addi_cost" method="post">
                    ...
                    <div id="addi_cost_load_div" style="display:none; background-color:rgb(255, 252, 247); border:1px solid #CCC"> addi_cost_load_div </div></form>
                     <?php }else{ ?>
					 <div id="addi_cost_toggle"  onclick="addi_cost_toggle_click()" style="border:1px solid #666; padding:6px; background-color:#205c82; text-align:center; color:#FFF;text-decoration: underline;" > <i class="fa fa-hand-o-right" style="color:#FFF"></i>&nbsp;&nbsp;&nbsp;Click here to adding special add-ons to this itinerary</div>
...
foreach($break_arr as $breakup)
{
							...
							if($break_chk!=0)
							{?>
								<div class="row" style="padding:6px; text-align:center; border-top: 2px solid #256582;">
		...
		<center>
			<table style='width:94%; border:1px solid #256582;' border='1'>
				<tr style='background-color: #2369A0; color:#FFF'>
					...
					<th style='padding:6px; width:12%; text-align:center'> Cost (Rs.)</th>
				</tr>
				<?php 
				$iv=1;
				foreach($break_arr_table as $inv)
				{
...
echo date_format($date,"d-M-Y");	
					...
					<th style='padding:6px; text-align:left'> 
						<?php echo convert_currency_text("Rs")." ".number_format(convert_currency($row_invoice['agnt_grand_tot']),2); ?></th>
				</tr>
				<?php } ?>
				<tr>
					<th style='padding:6px; text-align:center; font-size: 18px;' colspan='5'> Overall Trip Cost </th>
					<th style='padding:6px;  text-align:left; font-size: 18px;'>
					 <?php echo convert_currency_text("Rs")." ".number_format(convert_currency($whole_itin_amt),2)." /-"; ?><small>Only</small></th>
				</tr>
			</table>
			<?php 
	$addon_arr=array();
	$addon_det=array();
		...
		<p align="left" style="margin-top:5px"><strong style="font-size: 12px;color: #0700;"> Total Cost Including Special Add-ons On : </strong></p>
	    ...
	    <?php  } ?>
					...
					if($break_chk==0){?><input type="hidden" value="<?php echo $_GET['planid']; ?>" id='sub_planid' name="sub_planid" /><?php } 
					 $break_chk++;?>
                              <!--  <center><table style="width:95%; border:#DA832D 2px solid; background-color:#FFF4DF; color:#6B4D1E" class="table">
                                <tr><td width="18%">&nbsp;Creator</td><td width="3%">:</td><td width="25%"><?php echo $creator; ?></td>
                               <td width="18%">&nbsp;Reference Itinerary ID</td><td width="3%">:</td><td width="25%"><?php echo $row_orders['tr_name']."( For ". $row_orders['pax_cnt']."&nbsp;Pax )"; ?></td></tr>
                                <tr><td width="20%" >&nbsp;Total Traveling Days</td><td width="3%">:</td><td width="25%"><?php echo $row_orders['tr_days']; ?></td>			<td width="18%">&nbsp;Vehicle Infomation</td><td width="3%">:</td>
                                <td width="25%"><?php
								...
								else
								{
                                  ...
                                  <td> Transport Charge</td><td>:</td><td> <a href="javascript:void(0)" style="text-decoration:blink" class="flashit"><?php echo $row_orders['agnt_grand_tot']."/- Rupees"; ?></a></td></tr>
                                </table></center>-->
...
$totalRows_via_hspots =$via_hspots->rowCount();	
                            ...
                            <br>
                                	<p style="color:#F00; text-align:center ; font-weight:600">Package includes:</p>
