...
$query_fes = "SELECT * FROM dvi_front_settings";
$fes = mysql_query($query_fes, $divdb) or die(mysql_error());
$row_fes = mysql_fetch_assoc($fes);
$totalRows_fes = mysql_num_rows($fes);


if(isset($_POST['faq_forms']) && $_POST['faq_forms']=='faq_forms_val')
{
    mysql_select_db($database_divdb, $divdb);
	$query_faq = sprintf("insert into dvi_front_faq(faq_quest,faq_ans,status) values(%s,%s,%s)",
			 		GetSQLValueString($_POST['faq_quest'], "text"),
					GetSQLValueString($_POST['faq_answer'], "text"),
					GetSQLValueString('0', "int"));
	mysql_query($query_faq, $divdb) or die(mysql_error());
								...
								<div class="panel-body">
									<div class="tab-content">
										<div class="tab-pane fade in active" id="home">
                                       		    <?php 
mysql_select_db($database_divdb, $divdb);
$query_home= "SELECT * FROM dvi_front_home where status='0'";
$home = mysql_query($query_home, $divdb) or die(mysql_error());
$row_home = mysql_fetch_assoc($home);
$total_home = mysql_num_rows($home);
?>
										<div class="col-sm-12" id="" style="border:1px rgb(245, 241, 241) solid; margin:0px;background-color:#F7F7F7;">
                                        ...
                                        <?php if(trim($row_home['welcome_heading'])!=''){ echo "<strong>".$row_home['welcome_heading']."</strong>"; }else {?> <strong style="text-align:center; font-size:24px; color:#CCC;">Not given</strong><?php } ?></div>
                                            ...
                                            <div class="col-sm-9" id="welcome_desctarea">
                                         
                                            <textarea   class="summernote-sm" name="off_address"><?php if($row_home['welcome_desc']!=""){ echo $row_home['welcome_desc']; } else {echo "Not given";}?></textarea>
                                          </div>
                                            </div>
                                            <hr />
                                         ...
                                         <a id="feature_upd_btn" class="btn btn-default pull-right" style="display:none;" onclick="upload_home_dvi('F')"><i class="fa fa-upload"></i></a>
                                            ...
                                            <div class="col-sm-9" id="feature_desctarea">
                                            
                                            <textarea  class="summernote-sm" name="featurez"><?php echo $row_home['feature_desc']; ?></textarea></div>
                                            </div>
                                             <hr />
...
<?php 
mysql_select_db($database_divdb, $divdb);
$query_about = "SELECT * FROM dvi_front_settings where status='0'";
$about = mysql_query($query_about, $divdb) or die(mysql_error());
$row_about = mysql_fetch_assoc($about);
$total_about= mysql_num_rows($about);
if($total_about>0)
{
?>
                                            ...
                                            <div class="col-sm-12" style="background-color:" id="mission_tarea">
                                            
                                            <textarea class="summernote-sm">
                                            <?php if($row_about['mission']!=""){ echo $row_about['mission']; }else { echo "Not given";} ?></textarea>
                                            </div>
                                            ...
                                            <div class="col-sm-12"  id="vision_tarea">
                                            
                                            <textarea class="summernote-sm" >
                                            <?php if($row_about['vision']!=""){ echo $row_about['vision']; }else { echo "Not given";} ?></textarea>
                                            </div>
                                            ...
                                            <div class="col-sm-12"  id="services_tarea">
                                            
                                            <textarea class="summernote-sm">
                                            <?php if($row_about['services']!=""){ echo $row_about['services']; }else { echo "Not given";} ?></textarea>
                                            </div>
                                            ...
                                            <div class="col-sm-12"  id="aboutus_tarea">
                                           
                                            <textarea class="summernote-sm" >
                                            <?php if($row_about['aboutus']!=""){ echo $row_about['aboutus']; }else { echo "Not given";} ?></textarea>
                                            </div>
...
<?php } ?>
										...
										<div class="tab-pane fade" id="faq">
                                           ...
                                           <br><br><br>Insert Answer : </div>
                                            ...
                                            <?php 
mysql_select_db($database_divdb, $divdb);
$query_faq_view = "SELECT * FROM dvi_front_faq where status='0'";
$faq_view = mysql_query($query_faq_view, $divdb) or die(mysql_error());
//$row_faq_view = mysql_fetch_assoc($faq_view);
$total_faq_view = mysql_num_rows($faq_view);
   
											if($total_faq_view>0)
											{
												$f=1;
												while($row_faq_view = mysql_fetch_assoc($faq_view))
												{ ?>
                 ...
                 <table style="margin-top:10px;"><tr><td><a class="btn btn-default" style="margin-left:" onclick="update_me(<?php echo $f; ?>,<?php echo $row_faq_view['sno']; ?>)"><i class="fa fa-upload"></i></a>&nbsp;</td><td>
                    ...
                    <p id="edit_answ<?php echo $f; ?>" style="display:none;" ><label> Answer <?php echo $f;?></label><textarea class="form-control" rows="5" style="width:100%; resize:vertical" id="upd_answ<?php echo $f; ?>"  name="upd_answ<?php echo $f; ?>"><?php echo $row_faq_view['faq_ans']; ?></textarea></p>
                                        ...
                                        <div class="tab-pane fade" id="contact_us">
                                                              <?php 
mysql_select_db($database_divdb, $divdb);
$query_cnt= "SELECT * FROM dvi_front_settings where status='0'";
$cnt = mysql_query($query_cnt, $divdb) or die(mysql_error());
$row_cnt = mysql_fetch_assoc($cnt);
$total_cnt = mysql_num_rows($cnt);
?>
                                        ...
                                        <?php if(trim($row_cnt['location'])!=''){ echo "<strong>".$row_cnt['location']."</strong>"; }else {?> <strong style="text-align:center; font-size:24px; color:#CCC;">Not given</strong><?php } ?></div>
                                            ...
                                            <div class="col-sm-9">
                                         <div style="height:100px; border:1px solid #CCC; overflow-y:scroll" id="addr_descview"><?php if(trim($row_cnt['location_desc'])!=''){ echo "<strong>".$row_cnt['location_desc']."</strong>"; }else {?> <strong style="text-align:center; font-size:24px; color:#CCC;">Not given</strong><?php } ?></div>
                                         ...
                                         <a id="mail_upd_btn" class="btn btn-default pull-right" style="display:none;" onclick="upload_contact_dvi('M')"><i class="fa fa-upload"></i></a>
                                            ...
                                            <?php if(trim($row_cnt['email_desc'])!=''){ echo "<strong>".$row_cnt['email_desc']."</strong>"; }else {?> <strong style="text-align:center; font-size:24px; color:#CCC;">Not given</strong><?php } ?>
                                            </div>
                                         ...
                                         <a id="help_upd_btn" class="btn btn-default pull-right" style="display:none;" onclick="upload_contact_dvi('H')"><i class="fa fa-upload"></i></a>
                                            ...
                                            <div  id="help_descview" style="height:60px; overflow-y:scroll; border:1px solid #CCC;">
                                             <?php if(trim($row_cnt['phone_desc'])!=''){ echo "<strong>".$row_cnt['phone_desc']."</strong>"; }else {?> <strong style="text-align:center; font-size:24px; color:#CCC;">Not given</strong><?php } ?></div>
...
$ff=1;
			while($row_fdbks = mysql_fetch_assoc($fdbks))
			{
?>
...
function upload_home_dvi(str)
{
	...
	if(str=='W')
	{
		...
		tarea2=$('#welcome_desctarea .note-editable').html();
    //alert(tarea2);
		id='welcome';
    ...
    tarea2=$('#feature_desctarea .note-editable').html();
		id='feature';
	}
	
	var ty=8;
$.post('ADMIN/ajax_front_end.php?type='+ty,{'field1':field1,'field2':field2,'tarea1':tarea1,'tarea2':tarea2,},function(result)
	{
		$('#'+id+'_tarea').hide();
		...
		$('#'+id+'_view').show();
		//$('#'+id+'_desctarea').empty().prepend(tarea2);
		$('#'+id+'_descview').show();
$('#'+id+'_desctarea .note-editable').empty().prepend(tarea2);
$('.summernote-sm').summernote({height:'200px',width:'100%'});
...
function feedback_submit()
{
	...
	var cname=$('#client_name').val();
	var cfeed=$('#feedtarea').val();
	$.post('ADMIN/ajax_front_end.php?type='+ty,{'cname':cname,'cfeed':cfeed},function(result)
	{
		$('#insert_fdbk').hide();
		$('#view_fdbk').show();	
		$('#add_btn_fdbk').show();
...
{
	var ty=5;
	$.post('ADMIN/ajax_front_end.php?type='+ty,function(result)
	{
...
function update_me_fdbk(no,sno)
{
	...
	$.post('ADMIN/ajax_front_end.php?type='+ty,{'cname':clname,'cfeed':clfeed,'sno':sno},function(result)
	{
...
function update_about_us(str)
{
    ...
    tarea=$('#mission_tarea .note-editable').html();
	}else if(str=='V')
	{
		str1='vision';
		//tarea=$('#vision_tarea').val();
    tarea=$('#vision_tarea .note-editable').html();
    
	}else if(str=='S')
	{
		str1='services';
		//tarea=$('#services_tarea').val();
    tarea=$('#services_tarea .note-editable').html();
	}else if(str=='A')
	{
		str1='aboutus';
		//tarea=$('#aboutus_tarea').val();
    tarea=$('#aboutus_tarea .note-editable').html();
	}
	
	var ty=2;
	$.post('ADMIN/ajax_front_end.php?type='+ty,{'str1':str1,'tarea':tarea},function(result)
	{
		...
		$('#'+str1+'_upd').hide();
		$('#'+str1+'_btn').show();
    $('#'+str1+'_tarea .note-editable').empty().prepend(tarea);
$('.summernote-sm').summernote({height:'200px',width:'100%'});
		alert("Updated Successfully..");
...
function update_me(no,sno)
{
	...
	$.post('ADMIN/ajax_front_end.php?type='+ty,{'quest':qus,'answ':ans,'sno':sno},function(result)
	{
		var str1=' '+no+' ) '+qus;
		var str2=' Answer : '+ans;
