...
$timezone = new DateTimeZone("Asia/Kolkata" );
 ...
 if(isset($_POST['news_sett']) && ($_POST['news_sett']=="news_sett_val"))
 {
	...
	$insert_news=$conn->prepare("insert into news_scroller(news,from_date,to_date,images,priority)values(?,?,?,?,?)");
	$insert_news->execute(array($news,$from_dt,$end_dt,$attachment,$orders));
...
if(isset($_POST['updnews_sett']) && ($_POST['updnews_sett']=="updnews_sett_val"))
 {
    ...
    $news2 = $conn->prepare("SELECT * FROM news_scroller where sno=?");
	$news2->execute(array($_POST['ssno']));
	...
	$insert_news=$conn->prepare("UPDATE news_scroller set news=?, from_date=?, to_date=?, images=?, priority=? where sno=?");
	$insert_news->execute(array($news,$from_dt,$end_dt,$attachment,$orders,$_POST['ssno']));
...
<?php 
	...
	$news = $conn->prepare("SELECT * FROM news_scroller  ORDER BY sno ASC");
	$news->execute();
	//$row_news = mysql_fetch_assoc($news);
	$row_news_main=$news->fetchAll();
	...
	<div class="container-fluid">
           ...
           <a id="edit_id" class="btn btn-info " data-target="#news_settings" data-toggle="modal"><i class="fa fa-plus"></i>&nbsp; Add News</a>
						...
						<table class="table table-striped table-hover datatable-example" >
                            ...
                            <?php 
							$s=1;
							foreach($row_news_main as $row_news){ ?>
