...
function GetSQLValueString($theValue, $theType, $theDefinedValue = "", $theNotDefinedValue = "") 
{
 ...
 if(isset($_POST['pack_sett']) && ($_POST['pack_sett']=="pack_sett_val"))
 {
	...
	$insert_pack = $conn->prepare("INSERT dvi_packages(pack_name, pack_location, pack_img, pack_pdf, pack_desc, pack_grp)values(?,?,?,?,?,?)");
    $insert_pack->execute(array($pack_name,$pack_locat,$pack_imgs,$pack_fils,$pack_desc,$pack_categ));
...
if(isset($_POST['upd_pack_sett']) && ($_POST['upd_pack_sett']=="upd_pack_sett_val"))
 {
	     ...
	     $update_pack = $conn->prepare("UPDATE dvi_packages set pack_name=?, pack_location=?, pack_img=?, pack_pdf=?, pack_desc=?, pack_grp=?  where sno=?");
         $update_pack->execute(array($pack_name,$pack_locat,$pack_imgs,$pack_fils,$pack_desc,$pack_categ,trim($_POST['my_sno'])));
...
<div class="panel with-nav-tabs panel-info panel-square panel-no-border">
	...
	$packages = $conn->prepare("SELECT * FROM dvi_packages where pack_grp='DI' ORDER BY sno ASC");
	$packages->execute();
	//$row_packages = mysql_fetch_assoc($packages);
	$row_packages_main=$packages->fetchAll();
	$totalRows_packages =$packages->rowCount();
						...
						<table class="table table-striped table-hover datatable-example" >
                            ...
                            <?php 
							$s=1;
							foreach($row_packages_main as $row_packages){ ?>
                            ...
                            <td style="text-align:justifydefault_img">
                            	...
                            	echo $row_packages['pack_desc'];
                        	 	...
                        	 	<p data-toggle="tooltip" title="<?php echo substr($row_packages['pack_desc'],0,150000); ?>">
                        	 	<?php 
	...
	$packages = $conn->prepare("SELECT * FROM dvi_packages where pack_grp='IN' ORDER BY sno ASC");
	$packages->execute();
	//$row_packages = mysql_fetch_assoc($packages);
	$row_packages_main=$packages->fetchAll();
	$totalRows_packages = $packages->rowCount();
