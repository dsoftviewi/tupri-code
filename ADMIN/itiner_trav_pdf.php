...
<br><b style="margin-left:550px; font-size:12px;" > </b></td>
    </tr>
  </table>
</div>
	...
	if(!empty($dt_arr) && $chn != 0 && in_array($chn,$dt_cnt_arr))
								 {
									 ...
									 {
		$html.=  "<br><span style='font-weight:bold; color:green'>"."After Breakfast,Full DAYTRIP to ".$dt_arr[$row_trv['tr_from_cityid']][0]." (".$daytravel_dist." kms) : </span>";
...
$totalRows_hot1 = $hot1->rowCount();
	...
	if($row_trv['tr_from_cityid'] == $row_trv['tr_to_cityid'])
	{
		...
		if(!empty($dt_arr))
		{
			if(isset($dt_arr[$row_trv['tr_from_cityid']][0]))
			{
				$html.= "<br><br><span style='font-weight:bold; color:green'>"."After Breakfast,Full DAYTRIP to ".$dt_arr[$row_trv['tr_from_cityid']][0]."</span>";
...
else
{
	...
	<tr>
        <td width="14%" rowspan="2" style="background-color:#F5F5F5">
...
$totalRows_dvcity_hist =$dvcity_hist->rowCount();
	...
	{
     $html.='<tr style="background-color:; font-weight:600;padding:8px"><td width="35%" sytle="padding:8px">Visiting Spots</td><td width="65%" sytle="padding:8px">Opening and Closing Time</td></tr>';
		...
		for($hr=0;$hr<count($tot_spot_arr);$hr++)
		{
			if($tot_spot_arr[$hr]!='-' && $tot_spot_arr[$hr]!='')
			{
                ...
                $html.='<tr><td width="35%" sytle="padding:6px">'.$row_hstime['spot_name'].'</td><td width="65%">'.$row_hstime['spot_timings'].'</td></tr>';
			}
		}
           $html.='';
   }
...
$totalRows_sy_scd = $sy_scd->rowCount();
  ...
  <tr class="hide_border"> 
	...
	<table width="100%"  border="0" style="font-family: ; margin-top:20px; text-align: left; font-size: 14px">
      ...
      <tr>
        <td colspan="2" height="30px"><strong>Please tell us what you dislike.<br><br><br></strong></td>
        </tr>
      <tr>
...
$canvas->page_text(52, 98, "Pages : {PAGE_NUM} of {PAGE_COUNT}", $font, 8, array(0,0,0));
