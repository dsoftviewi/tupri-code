...
if(!isset($_GET['fdate']) && !isset($_GET['tdate']))
{
		...
		$sspro1 = $conn->prepare("SELECT * FROM stay_sched where stay_id =? and (`sty_date`>='".date('Y-m-d',?))."' and `sty_date`<='".date('Y-m-d',?))."') ORDER BY sno ASC ");
}
$sspro1->execute(array($_GET['planid'],$_GET['planid'],strtotime($_GET['fdate'],strtotime($_GET['tdate']));
