...
if(isset($_POST['submit_city']) && $_POST['submit_city']='submit_city_val')
{
	...
	$row_getcode = $getcode->fetch(PDO::FETCH_ASSOC);
 	...
 	$insertSQL1 = $conn->prepare("INSERT INTO dvi_cities (id ,country, region, name) VALUES (?,'IN', ?, ?)");
	$insertSQL1->execute(array(trim($_POST['new_city_idd']),$row_getcode['code'],$_POST['cityname']));
	...
	{
		$to_city=$f;
		$insertSQL1=$conn->prepare("INSERT INTO dvi_citydist (from_cityid, to_cityid, dist, status) values(?,?,'0',0)");
		$insertSQL1->execute(array($from_city,$to_city));
	}
	...
	if(trim($_POST['cdesc'])=='')
	{
	 ...
	 $insertSQL1 = $conn->prepare("INSERT INTO dvi_cities_history (cid, `cdesc` , status) VALUES (?,?, 0)");
     $insertSQL1->execute(array($row_id['id'],$_POST['cdesc']));
