        ...
        $pack_location.='~'.$_POST['pack_locat'.$j];
		 ...
		 if(trim($pack_imgs)!='')
		 {
			...
			$image_store = str_replace(' ', '', $image_store);
   		    ...
   		    $insert_pack = $conn->prepare("UPDATE dvi_packages set pack_name=?, pack_location=?, pack_img=?, pack_desc=? , p_d=?, day_title=?, day_locat=?, pack_grp=? where sno=?");
            $insert_pack->execute(array($pack_name,$pack_locat,$image_store,$pack_desc,$pack_d,$day_title,$pack_location,$pack_categ,trim($_POST['pack_sno'])));
...
<?php 
	...
	$packages = $conn->prepare("SELECT * FROM dvi_packages where status=0 and sno=?");
	$packages->execute(array($_GET['dvisno']));
